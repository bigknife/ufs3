package ufs3.integ.backup_server.visitor
...
import ufs3.integ.backup_server.visitor.BackupActor.{BackupNewFile, BackupWriteData, ReadCompleted, WriteCompleted}
import ufs3.integ.backup_server.visitor.PutActor.RunWithUFS3
import ufs3.interp.commons.Stack
import ufs3.kernel.commons.Layout.Bytes
...
class PutActor(coreConfig: Config) extends Actor {
  import BackupLogger._
  def receive: Receive = {
    case RunWithUFS3(key, ins) ⇒
      val _sender = sender()
      ...
      def write(): Unit = {
        Try {
          // todo , the ufs3 holder is common, but now in the httpServer space
          val p = _root_.ufs3.prog.write.isWriting[App.Op](_root_.ufs3.integ.httpServer.openReadWriteUFS3(coreConfig))
          ...
          case Success(false) =>
            val p = _root_.ufs3.prog.write[App.Op](key, ins, _root_.ufs3.integ.httpServer.openReadWriteUFS3(coreConfig))
            Try{
              ...
              case Failure(x) ⇒ //x.printStackTrace()
                _sender ! WriteCompleted(key, Some(x))
            }
        }
      }
      write()
...
object PutActor {
  final case class RunWithUFS3(key: String, in: InputStream)
  def props(coreConfig: Config): Props = Props(new PutActor(coreConfig))
}

class BackupActor(coreConfig: Config) extends Actor {
  import BackupLogger._
...
object BackupActor {
  ...
  def props(coreConfig: Config): Props = Props(new BackupActor(coreConfig))

  def backupActorRef(coreConfig: Config)(implicit system: ActorSystem): ActorRef = system.actorOf(props(coreConfig))
}
...
class BackupVisitor(coreConfig: Config, actorRef: ActorRef) extends AbstractConnectionVisitor {
  import Pointer._
  import BackupLogger._
